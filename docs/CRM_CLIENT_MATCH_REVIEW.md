...
The connection and mappings are installed. Bryan approved activation of the Calendar API and existing connection for verification while booking and syncing stay off. He subsequently generated and saved a replacement key, whose exact match was verified before deployment. Live CRM-to-Calendar service authentication now passes; invalid service credentials and missing human sign-in are rejected. Human-authenticated availability and live scheduling remain unverified. Agree on a disposable live task, open work slot and test-notification recipients before enabling a controlled pilot. Retain the established owner-only approval and protected-time rules. Deployment and verification evidence is recorded in [the rollout checklist](CRM_ROLLOUT.md).
