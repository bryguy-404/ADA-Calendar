...
The connection and mappings are installed. Bryan approved activation of the Calendar API and existing connection for verification while booking and syncing stay off, and those activation steps succeeded. Live authentication found two changed characters in the saved key, so a new key must be copied directly into the same CRM connection configuration before verification can pass. No replacement has been generated yet. After connection verification, agree on a disposable live task, open work slot and test-notification recipient before enabling a controlled pilot. Retain the established owner-only approval and protected-time rules.
