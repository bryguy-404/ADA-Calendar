...
The connection and mappings are installed with access disabled. Bryan's original key was saved in CRM's protected Railway configuration after he corrected one extra newline; no rotation was needed or performed. All five CRM connection settings are deployed successfully. Calendar's API-on and booking-off flags are staged, awaiting access confirmation together with the owner-controlled connection enable step. After connection verification, agree on a disposable live task, open work slot and test-notification recipient before enabling a controlled pilot. Retain the established owner-only approval and protected-time rules.
