...
The connection and mappings are installed and active. Bryan generated and saved a replacement key, whose exact match and live service authentication were verified. Invalid service credentials and missing human sign-in are rejected. His approved live pilot passed a clean-fit booking through the signed-in CRM, a conflict requiring approval with suggested openings, a fresh clean-fit alternative preview, and owner cancellation synchronized back to CRM. The test work is cancelled and reserves no time. Bryan's test emails were delivered; Kyle's test update was captured. Bryan subsequently authorized normal team email delivery; the Calendar worker's live mode and next successful scheduled invocation are verified. Existing captured pilot messages remain captured. Retain the established owner-only approval and protected-time rules. Deployment, task IDs and precise verification limits are recorded in [the rollout checklist](CRM_ROLLOUT.md).
